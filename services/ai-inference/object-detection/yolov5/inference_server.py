...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_txt:  # Write to file
                        xywh = (xyxy2xywh_TL(torch.tensor(xyxy).view(1, 4))).view(-1).tolist()  # normalized xywh
                        #line = (cls, *xywh, conf) # if save_conf else (cls, *xywh)  # label format
                        detections.append({"x":xywh[0], "y": xywh[1], "width": xywh[2], "height": xywh[3], "confidence": confidence_str, "class": "Turkey"})
            ...
            # Append the detections bbox to the image list
            image_detlist.append(detections)
    ...
    if len(image_detlist) == 1:
        return image_detlist[0]
    else:
        return image_detlist
...
async def upload_image(file: UploadFile = File(...)):
    try:
        ...
        save_path = f"./temp_uploaded_images/{file.filename}"
        ...
        # Parse results
        detections = detects #[{"x":30, "y": 30, "width": 40, "height": 40, "confidence": 0.9, "class": "Turkey"}]

#############################################################################################################
# 
# Track objects using yolov8 detection model and euclidean distance matching. Results are saved in local 
# directory. This file is part of the object tracking task. For convenience, it is kept in this folder.
# 
##############################################################################################################
...
while cap.isOpened():
    ...
    for result in results:
        ...
        for bbox in result.boxes:
            x1, y1, x2, y2 = bbox.xyxy[0].tolist()  # Bounding box coordinates
            confidence = bbox.conf[0].item()        # Confidence score
            class_id = int(bbox.cls[0].item())      # Class ID
            name = model.names[class_id]            # Class name
            cx = int((x1 + x2) / 2)
            cy = int((y1 + y2) / 2)
            centre_points_cur_frame.append((cx, cy))
            #print("FRAME N: ", count, " ", x1, y1, x2, y2, class_id, name)
